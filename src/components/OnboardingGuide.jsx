...
const PAGES = [
  {
    ...
    content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <p style={bodyStyle}>
          时间焦虑不是时间在流逝造成的。流逝是物理事实，没有什么可以对抗。
        </p>
        <p style={bodyStyle}>
          真正的来源更具体：<strong style={{ color: 'var(--accent-text)' }}>资源在减少，而我正在做的事与「最是我自己的方向」之间，夹角太大。</strong>
        </p>
        <p style={bodyStyle}>
          PRIME 借用《Rick and Morty》里的概念——在所有平行版本的你之中，存在一个最纯粹、最符合你自己定义的那个人。那个人是 Prime。
        </p>
        <p style={bodyStyle}>
          Prime 不是一个要抵达的终点，是一个<strong style={{ color: 'var(--text)' }}>方向</strong>。
        </p>
    ...
    content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{
          ...
          <span className="mono" style={{ fontSize: '22px', color: 'var(--accent-text)', letterSpacing: '0.02em' }}>
            P = ∫₀ᵀ s⃗(τ) · û_prime(τ) dτ
          </span>
        </div>
        <p style={bodyStyle}>
          <strong style={{ color: 'var(--text)' }}>P</strong> 是你生命在 Prime 方向上积累的总价值。这个公式有三个核心含义：
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {[
            {
              label: '点积 ·',
              text: '你每时每刻的行为向量 s⃗ 与 Prime 方向的投影。完全对齐时 P 高效增长；逆 Prime 时 P 主动减少——不是停滞，是真实扣分。',
            },
            {
              label: '积分 ∫',
              text: '意义不是在某个瞬间把握的，而是每一个真实的 dτ 的持续积累。没有一天的真实努力会消失，它永远加在积分里。',
            },
            {
              label: '有限性 T',
              text: '正因为 T 是有限常数，每个选择才有真实成本。有限性不是在催促你，它是让每个选择「不可替代」的前提。',
            },
          ].map(({ label, text }) => (
            <div key={label} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
  ...
  },
  {
    ...
    content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <p style={bodyStyle}>
          最初的公式有一个隐含假设：û_prime 是固定的。这是理想化。
        </p>
        <p style={bodyStyle}>
          现实里，你对自己的理解在不断加深——通过时间、经历和自我观察。二十岁的 Prime 和四十岁的 Prime 不会相同。这不是退步，而是你更了解自己了。
        </p>
        <div style={quoteStyle}>
          完整公式：P = ∫₀ᵀ s⃗(τ) · <strong>û_prime(τ)</strong> dτ
          <br />û_prime 成为时间的函数。
        </div>
        <p style={bodyStyle}>
          每次你真正认识了自己的一个新层次，你不是在否定过去，而是在<strong style={{ color: 'var(--accent-text)' }}>更新坐标系</strong>。
          过去在旧轴上积累的 P 是真实的——那是把你带到此刻的路径。
        </p>
        <p style={bodyStyle}>
          你随时可以在「我的」页面提交新的感悟，让 AI 帮你校准 Prime Profile。这是这个工具存在的核心功能之一。
        </p>
      </div>
    ),
  },
  {
    ...
    content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <p style={bodyStyle}>
          有时候人会经历一段漂流期——疲惫、妥协、被裹着走。焦虑容易冒出来：<em style={{ color: 'var(--text-secondary)' }}>我在变成 NPC，我在浪费时间。</em>
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', margin: '4px 0' }}>
          <div style={{ padding: '14px 16px', borderRadius: '8px', background: 'var(--red-soft)', border: '1px solid var(--border)' }}>
            ...
            <p style={{ ...bodyStyle, margin: 0, fontSize: '13px' }}>
              û_prime 没有变，s⃗ 偏离了。内在有撕裂感。点积在跑负数。<strong>这是真实的焦虑来源。</strong>
            </p>
          </div>
          <div style={{ padding: '14px 16px', borderRadius: '8px', background: 'var(--green-soft)', border: '1px solid var(--border)' }}>
            ...
            <p style={{ ...bodyStyle, margin: 0, fontSize: '13px' }}>
              û_prime 本身旋转了。此后每一个平静的 dτ，都是在新轴上的<strong>正积累</strong>。
            </p>
          </div>
        </div>
        <p style={bodyStyle}>
          焦虑只活在这个间隙里：行为指一个方向，Prime 轴指另一个，两个都不调整。
          PRIME 的自评和报告正是帮你识别这个间隙的工具。
        </p>
      </div>
    ),
  },
    ...
    content: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {[
          {
            step: '01',
            title: '完成问卷，锁定 Prime Profile',
            desc: '回答 5 个关于你自己的问题。AI 综合这些回答，生成你的 Prime Profile——这是你 û_prime 的初始定义。你可以在确认页调整后锁定。',
          },
          {
            step: '02',
            title: '每日：计划 → 日志 → 报告',
            desc: '每天列出你的计划，AI 对每项进行 Prime 分级（+3 到 -2）。填写日志时选择自评分数（-3 到 +3），系统计算语义对齐度并得出 ΔP。报告会冷静地告诉你今天发生了什么。',
          },
          {
            step: '03',
            title: '持续校准你的 Prime',
            desc: '在「我的」页面，你可以随时提交近期的感悟和变化。AI 会提出 Profile 更新建议，你审阅后确认。每次更新都会重新生成 Embedding，版本号递增。',
          },
        ].map(({ step, title, desc }) => (
